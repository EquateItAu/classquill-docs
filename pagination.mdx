...
| `/sessions` | **`starts_at` desc** | **No — see below** |

<Warning>
  `/sessions` is ordered by when the session is **scheduled to happen**, not by when it
  was booked. A session booked today for next month sorts above one booked today for
  tomorrow, and a session backfilled for a past date sorts near the bottom. Don't poll it
  to detect new bookings — use a [webhook](/webhooks) instead.
</Warning>
